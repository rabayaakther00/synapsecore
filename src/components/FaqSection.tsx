...
export const FaqSection: React.FC = () => {
  ...
  return (
    <section id="faq" className="py-24 bg-[#0B132B]/50 relative border-t border-slate-800/80">
        ...
        <div className="space-y-3.5">
          {generalFaqs.map((faq) => {
            ...
            return (
              <div
                key={faq.id}
                className="rounded-2xl border border-slate-800 bg-slate-900/80 overflow-hidden transition-all duration-200 shadow-sm backdrop-blur-xl hover:border-indigo-500/40"
              >
                <button
                  onClick={() => toggleFaq(faq.id)}
                  type="button"
                  className="w-full flex items-center justify-between p-5 text-left font-bold text-white text-base hover:text-indigo-300 transition-colors gap-4"
                ...
                >
                  ...
                  <div
                    className={`w-7 h-7 rounded-full bg-slate-950 flex items-center justify-center text-cyan-400 shrink-0 transition-transform duration-200 border border-slate-800 ${
                      isOpen ? 'rotate-180 bg-indigo-600 text-white border-indigo-400' : ''
                    }`}
                  >
